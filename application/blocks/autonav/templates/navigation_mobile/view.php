...
if (count($navItems) > 0) {
    echo '<ul id="skz-menu" class="nav nav-bold nav-lead flex-column my-7">'; //opens the top-level menu

    foreach ($navItems as $ni) {
        echo '<li class="nav-item ' . $ni->classes . '">'; //opens a nav item
        echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="nav-link lead-6 fw-800 width-fit-content mx-auto ' . $ni->classes . '">' . h($ni->name) . '</a>';
