<?php namespace Application\Block\AccordionContainer;
...
{
    public $btFieldsRequired = ['stack', 'accordioncontainerid'];
    protected $btTable = 'btAccordionContainer';
    ...
    {
        return t("Akkordion Container");
    }

    public function getSearchableContent()
    {
        $content = [];
        $content[] = $this->accordioncontainerid;
        $content[] = $this->class;
    ...
    public function view()
    {
        ...
        $stack = [];
        if ($stack_entries = $db->fetchAll('SELECT * FROM btAccordionContainerStackEntries WHERE bID = ? ORDER BY sortOrder ASC', [$this->bID])) {
    ...
    {
        $db = Database::connection();
        $db->delete('btAccordionContainerStackEntries', ['bID' => $this->bID]);
        parent::delete();
    }
    ...
    {
        $db = Database::connection();
        $stack_entries = $db->fetchAll('SELECT * FROM btAccordionContainerStackEntries WHERE bID = ? ORDER BY sortOrder ASC', [$this->bID]);
        foreach ($stack_entries as $stack_entry) {
            unset($stack_entry['id']);
            $db->insert('btAccordionContainerStackEntries', $stack_entry);
        }
    ...
    protected function addEdit()
    {
        ...
        $al->register('javascript', 'select2sortable', 'blocks/accordion_container/js_form/select2.sortable.js', [], $this->pkg);
                    ...
                    switch ($type) {
                        case 'update':
                            foreach ($values as $id => $data) {
                                $db->update('btAccordionContainerStackEntries', $data, ['id' => $id]);
                            }
                            break;
                        case 'insert':
                            foreach ($values as $data) {
                                $db->insert('btAccordionContainerStackEntries', $data);
                            }
                            break;
                        case 'delete':
                            foreach ($values as $value) {
                                $db->delete('btAccordionContainerStackEntries', ['id' => $value]);
                            }
                            break;
                    }
                }
    ...
    public function validate($args)
    {
        ...
        if (in_array("accordioncontainerid", $this->btFieldsRequired) && (trim($args["accordioncontainerid"]) == "")) {
            $e->add(t("The %s field is required.", t("Akkordion Container ID")));
        }
